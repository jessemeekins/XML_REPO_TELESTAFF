...
class Record:
    ...
    def __init__(self, eid:str|int, name:str, rank:str, position:str, company:str, time:str, verbose=False):
        ...
        # captures error message in a variable called "e"
            ...
            print(f'{e}+FUUUCKKKK')
    # For future usage, returns a formatted string. Customizable however you want this string to appear. 
    def __str__(self):
        return f"{self.eid}-{self.name}-{self.rank}-{self.position}-{self.paramedic}-{self.company_abr}-{self.time}"
